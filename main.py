...
def TakeCase(case, grille):
    print(case)
    grilleBtn[case]["text"] = 'O'
    grilleBtn[case]["command"] = None
...
def Game():
    ...
    for i in range(3):
        i = 0
        for j in range(3):
            entry = Entry(frm, borderwidth=0, relief="flat", highlightcolor="white")
            grilleBtn.append(ttk.Button(entry, text=" ").grid(column=j, row=i))
    i = 0
    for i in range(9):
        grilleBtn[i]["command"] = TakeCase(i, grille)
    Draw(grille)
    isPlay = True
    while isPlay:
        root.mainloop()
